...
const ChatMessages = ({ userId, userType }) => {
    ...
    useEffect(() => {
        const fetchChatDetails = async () => {
            try {
                ...
                const otherUserId = userType === "student" ? response.data.landlordId : response.data.studentId;

                const userResponse = await axios.get(`http://localhost:8080/api/users/${otherUserId}`);
                const propertyResponse = await axios.get(
                    `http://localhost:8080/api/properties/${response.data.propertyId}`
                );
    ...
    const sendMessage = async (e) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:80api/chats/${chatId}/messages`);
            setMessages(response.data);
        } catch (err) {
    ...
    if (loading && !chatDetails)
        return <div className="flex items-center justify-center h-full p-4 text-gray-500">Loading conversation...</div>;

    if (error) return <div className="flex items-center justify-center h-full p-4 text-red-500">{error}</div>;

    return (
        <div className="flex flex-col h-full">
            <div className="flex items-center p-3 border-b border-gray-200 bg-white shadow-sm">
                <button className="mr-2 p-1 rounded-full hover:bg-gray-100 md:hidden" onClick={goBack}>
            ...
            <form className="p-3 border-t border-gray-200 bg-white flex items-center" onSubmit={sendMessage}>
                <input
                    type="text"
                    value={newMessage}
